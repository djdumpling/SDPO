...
# Train SDPO rubric generation with Qwen3-4B-Instruct on LitBench preference pairs.
#
# Usage:
#   # 1. Preprocess LitBench twice so training and eval can use different reward shaping.
#   uv run --active --project .. python -m environments.dpo_to_rupo_verl.preprocess_dataset \
...
#
#   # 2. Run training from this directory or submit train_qwen4b_litbench.slurm.
#   ./run_sdpo_qwen4b_litbench.sh [experiment_name_suffix]

# -----------------------------------------------------------------------------
# Configuration
# -----------------------------------------------------------------------------

# Keep the dataset, parquet paths, and model overridable so the same launcher can
# be reused for ablations without editing the script.
...
# These defaults match the existing LitBench SDPO setup while keeping the run
# small enough for a 4B policy plus a colocated 4B judge.
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-16}
ROLLOUT_N=${ROLLOUT_N:-4}
LR=${LR:-5e-6}
ALPHA=${ALPHA:-0.5}

# LoRA keeps memory use predictable and makes this launcher usable on 4 training
# GPUs even when the judge process occupies a fifth GPU.
LORA_RANK=${LORA_RANK:-32}
LORA_ALPHA=${LORA_ALPHA:-32}

# Tensor parallelism is controlled by the SLURM wrapper, but direct shell runs
# can override it if fewer or more GPUs are allocated.
TP_SIZE=${TP_SIZE:-4}
export N_GPUS_PER_NODE=${N_GPUS_PER_NODE:-4}
export NNODES=${NNODES:-1}

# The reward function talks to an OpenAI-compatible judge endpoint supplied by
# the SLURM wrapper or by the user's environment for direct runs.
...
SUFFIX=${1:-"qwen4b_litbench_ha_with_baseline"}
...
if [[ ! -f "$PROJECT_ROOT/$TRAIN_DATA/train.parquet" || ! -f "$PROJECT_ROOT/$EVAL_DATA/test.parquet" ]]; then
    ...
    echo "  uv run --active --project .. python -m environments.dpo_to_rupo_verl.preprocess_dataset \\"
...
# Include the model and dataset in the experiment name so W&B and checkpoint
# paths remain interpretable after multiple LitBench variants have run.
...
echo "----------------------------------------------------------------"
echo "Starting SDPO Rubric Training (Qwen3-4B-Instruct + LoRA)"
