"""Standalone rubric XML extraction without verifiers dependency.

Used inside verl reward workers where the verifiers package may not be installed.
Mirrors the extraction behaviour of vf.XMLParser(fields=["analysis", "rubric"]).
"""
...
def extract_rubric_text(response: str) -> str | None:
    """Extract the content of the <rubric>...</rubric> block from a model response."""
...
def extract_analysis_text(response: str) -> str | None:
    """Extract the content of the <analysis>...</analysis> block from a model response."""
    cleaned = strip_think_tags(response)
...
def extract_score(response: str) -> float | None:
    """Extract an integer score from a <score>...</score> block in a judge response."""
    cleaned = strip_think_tags(response)
